...
use nom::{
    ...
    error::{context, convert_error, ContextError, ParseError, VerboseError, ErrorKind},
...
pub fn char1<T, E: ParseError<T>>(input: T) -> IResult<T, T, E>
where
  T: InputTakeAtPosition,
  <T as InputTakeAtPosition>::Item: AsChar,
{
  input.split_at_position1_complete(|item| item.as_char() == '"', ErrorKind::Many1)
}
...
fn main() {
    ...
    match root::<VerboseError<&str>>(&data) {
        ...
        Err(Err::Error(e)) | Err(Err::Failure(e)) => {
            println!("Error: {:?}", convert_error(data.as_str(), e))
        }
